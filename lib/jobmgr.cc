...
JobMgr::JobMgr(bool daemon, const paroc_string &challenge, const paroc_string &url): JobCoreService(challenge)
{
	counter=1;
	requestCounter=1;
	modelearn=true;
...
JobMgr::JobMgr(bool daemon, const paroc_string &conf, const paroc_string &challenge, const paroc_string &url): JobCoreService(challenge)
{
	counter=1;
	requestCounter=1;
	modelearn=true;
			...
			for (int i=count;i<howmany;i++) if (!jobcontacts[i].IsEmpty())
				{
					try
					{
						...
						if (jobmgr.ExecObj(objname, od, sz, tmpids, localservice, objcontacts+count)!=0)
						{
