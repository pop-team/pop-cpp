...
{
	/**
	 * Asynchronous allocation of the object allows POP-C++ application to set up faster as normal sequential object construction.
	 * The code added here is working with some additional code generated by the POP-C++ parser to avoid invocation of methods
	 * while the parallel object is not fully created. 
	 */
	 
	// Start a thread to perform the standard allocation
	/*pthread_attr_t attr;
   pthread_attr_init(&attr);
   pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);	
	
	// Create the thread in charge of the allocation of the parallel object
	int ret;
	ret = pthread_create(&_popc_async_construction_thread, &attr, AllocatingThread, this);
   if(ret != 0)
	{
		popc_interface_log("Asynchronous thread couldn't be initialized\n");
		pthread_attr_destroy(&attr);
		return;	// Avoid further work in case the thread couldn't have been initialized
	}
