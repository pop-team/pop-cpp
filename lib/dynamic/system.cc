        ...
        try {
            if (normalExit) {
                ...
                while ((count = mgr->CheckObjects(loop > 5)) > 0) {
                    ...
                    LOG_INFO("%d parallel objects remain. Awaiting %d s. Type Ctrl+C to kill all remaining objects",
                             count, timeout);
                    popc_sleep(timeout);
                    oldcount = count;
                }
                ...
                LOG_INFO(
                    "Main routine did not exit normally or did not return 0. This is treated as an error by POP-C++. "
                    "Killing all objects.");
                mgr->KillAll();
            }
