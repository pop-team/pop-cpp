  ...
  try 
      ...
      while (fgets(buf,1023,stdin)!=NULL)
	{
	  if (strncmp(buf,"done",4)==0)
	    {
	      paroc_accesspoint *jobcontacts=new paroc_accesspoint[howmany];
	      int *reserveIDs =new int[howmany];
	      int count=1;
	      printf("Do resource allocation now...\n");
	      if (jobmgr.AllocResource(paroc_system::appservice, objname, *od, howmany, jobcontacts, reserveIDs))
		{
