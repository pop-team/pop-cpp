...
/*
  ...
  Note by LWK: The factory should generate the parent class pop_combox and not have 3 different methods for uds, mpi and
  socket
 */
...
void pop_combox_factory::GetNames(std::string& prots) {
    prots = std::accumulate(list.begin(), list.end(), std::string(),
...
bool pop_combox_factory::Register(const char* name, int metrics, COMBOX_CREATOR creator) {
    ...
    auto it =
        std::find_if(list.begin(), list.end(), [metrics](combox_factory_struct& t) { return metrics < t.metrics; });
