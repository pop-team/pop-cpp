...
int paroc_broker::Run()
{
  ...
  paroc_array<paroc_string> allurl(comboxCount);
  paroc_array<paroc_string> allprot(comboxCount);
  
  for(i=0; i<comboxCount; i++)
    {
      (*comboxArrayPtr)[i]->GetUrl(allurl[i]);
      (*comboxArrayPtr)[i]->GetProtocol(allprot[i]);
    }
  ...
  for(i=0; i<comboxCount; i++)
    {
      ...
      paroc_combox *tmp=fact->Create(allprot[i]);
      if (tmp!=NULL) 
	{
	  tmp->SetTimeout(100000);
	  if (tmp->Create(0,false) && tmp->Connect(allurl[i]))
	    {
      ...
      if (shouldCancel) ptArray[i]->cancel(); else  delete ptArray[i];
