...
bool paroc_combox_socket::GetUrl(paroc_string & accesspoint)
{
  ...
  int iplist[10];
  int n=paroc_system::GetIP(iplist,10);
  if (n<=0)
    {
  ...
  for (int i=0;i<n;i++)
    {
      ...
      v4= v4>>8;
      if (i<n-1) sprintf(elem,"%s://%d.%d.%d.%d:%d ",(const char *)prot,v4,v3,v2,v1,port);
      else sprintf(elem,"%s://%d.%d.%d.%d:%d",(const char *)prot,v4,v3,v2,v1,port);
