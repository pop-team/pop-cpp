...
GlobusJobMgr::GlobusJobMgr(bool daemon, const paroc_string &conf, const paroc_string &challenge, const paroc_string &url) : JobMgr(daemon, conf,challenge, url)
{
  if (!Query("gramcontact",globus_gram))
    {
      globus_gram=paroc_system::GetHost();
    }
  else 
    {
      if (*globus_gram=='/')
...
try 
  {
    GlobusAppCoreService globus_job(localservice);
    int job_id;
    ret=globus_job.SubmitJob(globus_gram,objname,howmany, platform, objglobal,callback,env,job_id);
    if (ret!=0)
      {
...
bool GlobusJobMgr:: MatchUser(paroc_accesspoint &localservice)
{
  ...
  GlobusAppCoreService user_entity(localservice);
