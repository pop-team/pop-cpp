...
bool NodeInfoMap::HasContact(const POPString &contact) {
    ...
    for(auto& v : hashmap){
        if(paroc_utils::SameContact(paroc_string(v.first).c_str(), contact.c_str())){
            return true;
        }
    }
...
//  assert(fd0==0 && fd1==1 && fd2==2);
    ...
    strcpy(mycontact,(GetAccessPoint()).GetAccessString().c_str());
...
int JobMgr::Query(const POPString &type, POPString  &val) {
    char tmp[1024];
    if(paroc_utils::isEqual(type.c_str(),"platform")) {
        val=paroc_system::platform;
        return 1;
    }
    if(paroc_utils::isEqual(type.c_str(),"host")) {
        val=paroc_system::GetHost();
        return 1;
    }
    if(paroc_utils::isEqual(type.c_str(),"jobs")) {
        //  Update();
        mutex {
    ...
    if(paroc_utils::isEqual(type.c_str(),"joblist")) {
        //  Update();
        val="";
    ...
    if(paroc_utils::isEqual(type.c_str(),"pausejobs")) {
        //  Update();
        mutex {
    ...
    if(paroc_utils::isEqual(type.c_str(),"power_available")) {
        //  Update();
    ...
    for(auto& t : info){
        if(paroc_utils::isEqual(type.c_str(),t.name.c_str())) {
            val=t.val;
            return 1;
        }
    }
                ...
                for(int i=count; i<howmany; i++) if(!jobcontacts[i].IsEmpty() && fitness[i]>0) {
                        try {
                            ...
                            for(int j=i+1; j<howmany; j++){
                                if(fitness[j]>0 && ac.GetAccessString() == jobcontacts[j].GetAccessString()){
            ...
            for(int i=count; i<howmany; i++) {
                if(!jobcontacts[i].IsEmpty()) {
                    try {
                        ...
                        for(int j=i+1; j<howmany; j++) {
                            if(ac.GetAccessString() == jobcontacts[j].GetAccessString()) {
                                jobcontacts[j].SetAccessString("");
...
//   Update();
    ...
    for(jobindex = 0; jobindex < howmany; jobindex++) {
        ...
        n_ac.SetAccessString(std::string(ni->nodeId));
    ...
    for(int i=howmany-1; i>=0; i--) {
        ...
        if(t>0 && !(jobcontacts[pos].IsEmpty())) {
            ...
            try {
                ...
                LOG_ERROR( "[JM] Fail to cancel reservation #%d on %s",reserveIDs[pos],jobcontacts[pos].GetAccessString().c_str());
            }
        }
        reserveIDs[pos]=id;
...
bool JobMgr::Forward(const paroc_accesspoint &localservice, const POPString &objname, const paroc_od &od, int howmany, float *fitness, paroc_accesspoint *jobcontacts, int *reserveIDs, int requestInfo[3], int iptrace[MAX_HOPS], int tracesize) {
    ...
    for(auto& contact : neighbors.GetContacts()){
        ...
        try {
            ...
            LOG_CORE( "[JM] Exception on contact %s", contact.c_str());
    ...
    if(modelearn) {
        ...
        for(int i=0; i<howmany; i++){
            if(index[i]==-1) {
                ...
                if(t.Length() <= 0 || paroc_utils::isEqual(t.c_str(),local.c_str())) {
                    continue;
                }
                NodeInfo info;
...
void JobMgr::SelfRegister() {
    ...
    for(auto& tmp : parents){
        try {
            ...
            LOG_ERROR( "[JM] can not register the local job service [%s] to %s",GetAccessPoint().GetAccessString().c_str(), tmp.GetAccessString().c_str());
        }
    }
...
int JobMgr::Exec(char **arguments, char *env[], int &pid, POPString popAppId, POPString reqID) {
    ...
    if(strcmp(first, pt_java_exec) != 0) {
        ...
        if(Query("jobmgr",str) && str.c_str()!=NULL) {
...
int JobMgr::ExecObj(const POPString  &objname, const paroc_od &od, int howmany, int *reserveIDs, const paroc_accesspoint &localservice, paroc_accesspoint *objcontacts) {
    ...
    *curenv=NULL;
    ...
    try {
        ...
        if(!code.QueryCode(objname,paroc_system::platform,mycodefile) || mycodefile.c_str() == NULL) {
            CancelReservation(reserveIDs,howmany);
    ...
    argv[n++]=popc_strdup(jobservice_arg.c_str());
    ...
    if(!localservice.IsEmpty()) {
        ...
        argv[n++]=popc_strdup(localservice_arg.c_str());
    }
