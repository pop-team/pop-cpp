...
bool DTreeNode::Decompose(CListDTreeNodePtr &decompose) {
    if(decomposition.IsEmpty() || !ComputeTimeConstraints()) {
        return false;
    }
...
//Now find end step of each task...
    ...
    while(pos!=NULL) {
        DTreeNode *node=decomposition.GetNext(pos);
        if(node->GetEndStep()>=nstep) {
            node->SetEndStep(nstep-1);
        }
...
//Now re-generate sequential diagram into array structures...
    ...
    if(!model.Init(nstep,ntask,complexity,start,end)) {
        return false;
    }
    model.Solve(100000);
...
bool DTreeNode::UpdateProbNext(ProbObj *p) {
    if(!CreateNextDependency(p)) {
        ...
        while(pos!=NULL) {
            DTreeNode *t=decomposition.GetNext(pos);
            assert(t!=NULL);
            if(t->GetTotalStep()!=t->GetEndStep()+1) {
                continue;
            }
            if(!t->UpdateProbNext(p)) {
                return false;
            }
        }
        return true;
...
bool DTreeNode::UpdateProbDependency(DTreeNode *prev) {
    if(sequential!=NULL) {
        return prev->UpdateProbNext(sequential);
    }
    ...
    while(pos!=NULL) {
        ...
        if(t->GetStartStep()==0) {
            if(!t->UpdateProbDependency(prev)) {
                return false;
            }
        }
    }
...
bool DTreeNode::UpdateChildDependency() {
    ...
    while(pos!=NULL) {
        ...
        if(!child->UpdateChildDependency()) {
            return false;
        }
        ...
        while(pos1!=NULL) {
            ...
            assert(t!=NULL);
            if(!child->UpdateProbDependency(t)) {
                return false;
            }
        }
    }
...
bool DTreeNode::InitProbDependency() {
    if(sequential!=NULL) {
        ...
        while(pos!=NULL) {
            ...
            assert(t!=NULL);
            if(!t->UpdateProbNext(sequential)) {
                return false;
            }
        }
        return true;
