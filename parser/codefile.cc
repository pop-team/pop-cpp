...
void CodeFile::FindAllBaseClassName(Class &t, CArrayString & bases, bool virtualBaseOnly)
{
  ...
  if (virtualBaseOnly)
    {
      tmp.SetSize(index);
      for (int i=0;i<index;i++) strcpy(tmp[i], bases[i]);
    }

  CArrayString &allbases=(virtualBaseOnly)? tmp : bases;
  ...
  while (1)
      ...
      for (int i=0;i<n;i++) 
	{
	  ...
	  for (int j=0;j<sz;j++) if (paroc_utils::isEqual(allbases[j],b.basename))
	    {
	      existed=true;
	      break;
	    }
	  if (!existed)
	    {
	      strcpy(allbases[actual_n],b.basename);
	      actual_n++;

	      if (virtualBaseOnly && b.baseVirtual)
		{
		  strcpy(bases[actual_n1],b.basename);
		  actual_n1++;
		}
	    }
	}
      ...
      if (virtualBaseOnly)
	{
      ...
      if (index<actual_n)
	{
	  cl=FindClass(allbases[index]);
	  if (cl==NULL)
	    {
	      fprintf(stderr,"ERROR: declaration of parclass %s not found!\n", allbases[index]);
	      exit(1);
	    }
	  index++;
	} 
...
DataType *CodeFile::FindDataType(char *name)
{
