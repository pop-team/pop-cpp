...
char *Compile(char *preprocessor, char *popcpp, char *cpp, char *pre_opt[], char *cpp_opt[], char *source, char *dest, bool usepipe, bool client, bool broker, bool warning, bool implicitpack, bool popcppcomp, bool noasyncallocation, bool kcomputer, bool collective, bool advanced){
	...
	char pseudodynamic[] = "-pseudo-dynamic"; 
...
//    if (collective) cmd1[countparoc++] = xmpoption;  
//    if (advanced) cmd1[countparoc++] = advancedoption;
...
char mpicxx[1024] = "/usr/local/openmpi/bin/mpicxx";
char mpicpp[1024] = "/usr/local/openmpi/bin/mpicxx -E";  
  ...
  if ((tmp = paroc_utils::checkremove(&argc, &argv, "-version")) != NULL) {
	...
	asyncallocation = (paroc_utils::checkremove(&argc, &argv, "-async-allocation") != NULL);
  ...
  kcomputer = (paroc_utils::checkremove(&argc, &argv, "-kcomputer") == NULL);
  enablexmp = (paroc_utils::checkremove(&argc, &argv, "-xmp") != NULL);
  enablempi = (paroc_utils::checkremove(&argc, &argv, "-mpi") != NULL);  
  enableadvanced = (paroc_utils::checkremove(&argc, &argv, "-advanced") != NULL);  
  enablepsdyn = (paroc_utils::checkremove(&argc, &argv, "-pseudo-dynamic") != NULL);  
  ...
  for (int i = 1; i < argc; i++) {
	...
	char tmplibfile[1024];
      ...
      if(enablexmp || enableadvanced || enablempi) {
