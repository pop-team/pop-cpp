...
bool Param::DeclareParam(char* output, bool header, size_t buffer_length) {
    if (mytype == nullptr) {
        return false;
    }
    if (isConst) {
        snprintf(output, buffer_length, "const ");
        ...
        if (allocmem && paramSize != nullptr && nptr) {
            ...
            if (nptr == 1 && base->IsParClass()) {
                snprintf(decl, sizeof(decl),
                         "pop_interface_container<%s> __pop_container_%s(%s);\n%s =__pop_container_%s;\n", basetype,
                         name, paramSize, name, name);
            } else {
                char tmpstr[1024];
...
bool Param::Marshal(char* bufname, bool reformat, bool inf_side, std::string& output) {
    ...
    *sizeinfo = 0;
    ...
    if (marshalProc != nullptr) {
        char tmpcode[1024];
        snprintf(tmpcode, sizeof(tmpcode), "%s(%s,%s, %s, %d, 0);\n", marshalProc, bufname, tmpvar,
                 (*sizeinfo == 0) ? "0" : sizeinfo, flags);
        output += tmpcode;
    } else {
...
bool Param::UnMarshal(char* bufname, bool reformat, bool alloc_mem, bool inf_side, std::string& output) {
    ...
    *sizeinfo = 0;
    ...
    if (marshalProc != nullptr) {
        ...
        snprintf(tmpcode, sizeof(tmpcode), "%s(%s,%s,%s,%d,%s);", marshalProc, bufname, tmpvar,
                 (*sizeinfo == 0) ? "0" : sizeinfo, flags, (inf_side ? "0" : "&_internal_mem"));
        output += tmpcode;
    } else {
...
void ObjDesc::Generate(char* code, size_t buffer_length) {
    code[0] = 0;
    if (odstr == nullptr) {
        return;
    }
    snprintf(code, buffer_length, "%s", "\n  ");
    strcat(code, odstr);
}
 ...
 * @return 0 in case of succes, -1 is the return argument cannot be marshalled, the number of the parameter that cannot
 * be marshalled
 */
int Method::CheckMarshal() {
    ...
    for (int i = 0; i < n; i++) {
        if (params[i]->GetType()->IsParClass() && !params[i]->IsRef()) {
            fprintf(stderr,
                    "%s:%d: POP-C++ Error in %s::%s : parallel object '%s' must be passed as reference to the remote "
                    "object.\n",
...
void Method::GenerateReturn(std::string& output, bool header, bool interface) {
    ...
    if (returnparam.IsRef() && !returnparam.GetType()->IsParClass()) {
        fprintf(
            stderr,
            "%s:%d: POP-C++ Error in %s::%s : methods of parallel objects can not technically return a reference.\n",
            GetClass()->GetFileInfo(), line, GetClass()->GetName(), (const char*)name);
        exit(1);
    }
...
void Method::GenerateArguments(std::string& output, bool header) {
    ...
    for (int j = 0; j < nb; j++) {
        Param& p = *(params[j]);
        p.DeclareParam(tmpcode, header, sizeof(tmpcode));
        if (j < nb - 1) {
            strcat(tmpcode, ", ");
        }
        output += tmpcode;
...
void Method::GenerateClient(std::string& output) {
    ...
    if (waitreturn) {
        ...
        if (!GetClass()->is_collective()) {
            snprintf(tmpcode, sizeof(tmpcode),
                     "\t{\n\t\tif (!__pop_buf->Recv((*__pop_combox), _popc_connection)) "
                     "pop_exception::pop_throw(\"Buffer "
        ...
        for (j = 0; j < nb; j++) {
            if (params[j]->OutParam() && !params[j]->GetType()->IsParClass()) {
                Class* cl = GetClass();
                fprintf(stderr,
                        "%s:%d: POP-C++ Error in %s::%s: Asynchronous method cannot have an output (parameter %s must "
                        "be declared with [in])\n",
        ...
        // Added for new communication support
        snprintf(tmpcode, sizeof(tmpcode), "_popc_connection->reset();\n}\n");
        output += tmpcode;
#endif
    }
}
...
void Method::GenerateBrokerHeader(std::string& output) {
    ...
    char str[1024];
    snprintf(str, sizeof(str), "\nvoid Invoke_%s_%d(pop_buffer &__pop_buf, pop_connection *__interface_output);", name,
             id);
    output += str;
}
...
void Method::generate_broker_header_pog(std::string& output) {
    ...
    char str[1024];
    snprintf(str, sizeof(str), "\n  void Invoke_%s_%d(pop_buffer &_popc_buffer, pop_connection *_popc_connection);",
             name, id);
    output += str;
}
    ...
    if (cl->IsCoreCompilation() || !cl->IsBasePureVirtual()) {  // ADDED FOR 2.0.3 Create constructor and stuff only if
        ...
        if (type == METHOD_CONSTRUCTOR) {
            ...
            if (GetClass()->is_collective()) {
                ...
                snprintf(methodcall, sizeof(methodcall),
                         "\n%s%s * _pop_obj = dynamic_cast<%s%s *>(obj);\ntry {\n_pop_obj->%s(", clname, OBJ_POSTFIX,
                         clname, OBJ_POSTFIX, name);
            }
        } else {
            char retdecl[1024];
            char tmpvar[1024];
            snprintf(tmpvar, sizeof(tmpvar), "%s", returnparam.GetName());
            ...
            if (returnparam.GetType()->IsParClass()) {
                ...
                if (GetClass()->is_collective()) {
                    ...
                    snprintf(methodcall, sizeof(methodcall),
                             "\n%s;\n%s%s * _pop_obj=dynamic_cast<%s%s *>(obj);\ntry {\n%s=_pop_obj->%s(", retdecl,
                             clname, OBJ_POSTFIX, clname, OBJ_POSTFIX, tmpvar, name);
                }
            }
            haveReturn = true;
        }
        ...
        for (int j = 0; j < nb; j++) {
            ...
            if (GetClass()->is_collective()) {
                snprintf(str, sizeof(str), "_popc_buffer");
            } else {
                snprintf(str, sizeof(str), "__pop_buf");
            }
        ...
        if (cl->IsWarningEnable()) {
            printf(
                "POP-C++ Warning: %s is an abstract parclass. Be aware that only the final class (parallel object) "
                "will keep this semantic.\n",
                clname);
        }
...
Constructor::Constructor(Class* cl, AccessType myaccess) : Method(cl, myaccess) {
    snprintf(name, sizeof(name), "%s", cl->GetName());
}
...
void Constructor::GenerateClientPrefixBody(std::string& output) {
    ...
    if (!GetClass()->IsCoreCompilation() && GetClass()->IsAsyncAllocationEnabled()) {
        snprintf(tmpcode, sizeof(tmpcode), "\n");
        output += tmpcode;
        od.Generate(tmpcode, sizeof(tmpcode));  // Generates the object description
        output += tmpcode;
#ifdef __APPLE__
        snprintf(tmpcode, sizeof(tmpcode),
        ...
        snprintf(tmpcode, sizeof(tmpcode), "\n  ");
        od.Generate(tmpcode, sizeof(tmpcode));
        strcat(tmpcode, "\n");
        output += tmpcode;
    ...
    if (!GetClass()->IsCoreCompilation() && GetClass()->IsAsyncAllocationEnabled()) {
        ...
        snprintf(tmpcode, sizeof(tmpcode),
                 ...
                 "%%s\\n\", ex.what());\n delete arguments;\n throw; \n    }\n   "
                 "      catch(...) {\n                     printf(\"ERROR: Exception in asynchronous "
                 "allocation\\n\");\n delete arguments;\n throw; \n    }\n   "
                 "delete arguments;\n  return 0;\n  }\n}\n");
        output += tmpcode;
    }
