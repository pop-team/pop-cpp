 ...
 * P.Kuonen   29/6/2011 Modify to print all exceptions raised in methods
 */
...
void Method::GenerateBroker(CArrayChar &output) {
    ...
    if(type==METHOD_CONSTRUCTOR) {
        ...
        if(returnparam.isRef) {
            sprintf(tmpvar,"&%s",returnparam.GetName());
        } else {
...
bool Method::hasOutput() {
    if(MethodType()==METHOD_NORMAL) {
        if(!returnparam.GetType()->Same("void")) {
            return true;
        }
    }
