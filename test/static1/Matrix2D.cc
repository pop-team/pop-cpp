...
Matrix2D::Matrix2D(int line, int col) {
    ...
    value = reinterpret_cast<ValueType*>(malloc((dataSize + 1) * ValueTypeSize)) if (value == NULL) {
        printf("\nMEMORY OVERFLOW !!!!\n");
        exit(0);
    }
    value[dataSize] = 0;
...
void Matrix2D::free() {
    ...
    if (value != NULL) {
        if (shared != NULL) {
            if (shared[dataSize] < 0.9)
                delete shared;
            else
                shared[dataSize] = shared[dataSize] - (ValueType)1;
        } else {
            if (value[dataSize] < 0.9)
                delete value;
            else
                value[dataSize] = value[dataSize] - (ValueType)1;
        }
    }
    value = NULL;
    shared = NULL;
...
void Matrix2D::showState(const char* s, bool all) {
    ...
    if (shared != NULL)
        printf("ref=%f, ", shared[dataSize]);
    else if (value != NULL)
        printf("ref=%f, ", value[dataSize]);
    printf("shared=%p", shared);
    if (value != NULL)
        printf(" at %p", (&value[dataSize]));
    printf("\n");
    if (all)
        if (value != NULL)
            display();
...
int Matrix2D::getLines() {
    return nbLine;
}
int Matrix2D::getCols() {
    return nbCol;
}
