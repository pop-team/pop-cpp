...
bool POPXMPObject_popcobject_Broker::invoke(unsigned method[3], pop_buffer& _popc_broker,
                                            pop_connection* _popc_connection) {
    if (*method == CLASSUID_POPXMPObject) {
        switch (method[1]) {
            case 10:
                Invoke_POPXMPObject_10(_popc_broker, _popc_connection);
                return true;
            case 13:
                Invoke_execute_xmp_1_13(_popc_broker, _popc_connection);
                return true;
            case 14:
                Invoke_set_value_14(_popc_broker, _popc_connection);
                return true;
            default:
                return false;
        }
...
POPXMPObject_popcobject_Broker::POPXMPObject_popcobject_Broker() {
    ...
    static popc_method_info _popc_minfo[3] = {
        {10, (char*)"POPXMPObject"}, {12, (char*)"execute_xmp_1"}, {13, (char*)"set_value"}};
    add_method_info(CLASSUID_POPXMPObject, _popc_minfo, 3);
}

// Collective, therefore the last process send a response
void POPXMPObject_popcobject_Broker::Invoke_POPXMPObject_10(pop_buffer& _popc_buffer,
                                                            pop_connection* _popc_connection) {
...
void POPXMPObject_popcobject_Broker::Invoke_execute_xmp_1_13(pop_buffer& _popc_buffer,
                                                             pop_connection* _popc_connection) {
...
void POPXMPObject_popcobject_Broker::Invoke_set_value_14(pop_buffer& _popc_buffer, pop_connection* _popc_connection) {
    int val;
...
POPC_GroupBroker* POPXMPObject_popcobject_Broker::_init() {
    return new POPXMPObject_popcobject_Broker;
}
