...
ParObject::ParObject()
{
	printf("Structparam2: Creating object ParObject on localhost\n");
}

ParObject::ParObject(POPString machine)
{
	printf("Structparam2: Creating object ParObject on %s\n",(const char *)POPSystem::GetHost());
}

ParObject::ParObject(float f)
{
	printf("Structparam2: Creating object ParObject with power %f\n",f);
}

ParObject::~ParObject()
{
	printf("Structparam2: Destroying the object ParObject... %d, %d\n",  theData.GetMyData(), theData.GetInternalData().GetInternalData());
}
