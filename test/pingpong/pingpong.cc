...
Pong::Pong(paroc_string enc, paroc_string prot)
{
...
{
  strcpy(pong, paroc_system::GetHost());
}
int Pong::Reset()
{
  count=0;
...
Ping::Ping(paroc_string enc, paroc_string prot, int p): test(enc, prot)
{
  rprintf("Ping on %s\n", (const char *)paroc_system::GetHost());
}
...
{
  strcpy(ping,paroc_system::GetHost());
...
int main(int argc, char **argv)
{
  ...
  paroc_string prot;
  char m1[256],m2[256];
  char *t=paroc_utils::checkremove(&argc, &argv,"-encoding=");
  if (t!=NULL) enc=t;
  
  t=paroc_utils::checkremove(&argc, &argv,"-protocol=");
  if (t!=NULL) prot=t;
