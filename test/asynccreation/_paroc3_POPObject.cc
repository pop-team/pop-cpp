...
# 2 "POPObject.cc" 2
# 10 "POPObject.cc"
POPObject__parocobj::POPObject__parocobj()
{__POPThis_POPObject = new POPObject(GetAccessPointForThis());
   paroc_system::_popc_cout.str(""); paroc_system::_popc_cout << "POPCobject created (by JobMgr) on machine:" << (const char*)POPSystem::GetHost() << std::endl; rprintf(paroc_system::_popc_cout.str().c_str());
}

POPObject__parocobj::~POPObject__parocobj()
{
   paroc_system::_popc_cout.str(""); paroc_system::_popc_cout << "POPCobject: on machine:" << (const char*)POPSystem::GetHost() <<" is being destroyed" << std::endl; rprintf(paroc_system::_popc_cout.str().c_str());
}

void POPObject__parocobj::firstMethod(){
 paroc_system::_popc_cout.str(""); paroc_system::_popc_cout << "First method is called on" << (const char*)POPSystem::GetHost() << std::endl; rprintf(paroc_system::_popc_cout.str().c_str());
}

void POPObject__parocobj::secondMethod(){
 paroc_system::_popc_cout.str(""); paroc_system::_popc_cout << "Second method is called on" << (const char*)POPSystem::GetHost() <<std::endl; rprintf(paroc_system::_popc_cout.str().c_str());
}


# 28 "POPObject.cc"
void paroc_registerbroker() {
POPObject__parocobjBroker::_fact.Test("POPObject");
}
bool CheckIfPacked(const char *objname)
{
	if (objname==0) return false;

	if (paroc_utils::isEqual(objname, "POPObject")) return true;
	return false;
}
